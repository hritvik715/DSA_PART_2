...
public:
    ...
    //postorder ko by reference lena hai kuki mai nhi chahta ek element ko baar baar process karo
    ...
    int &postIndex, int inorderStart, int inorderEnd, int size) {
        ...
        postIndex--;//(minus-minus kuki right to left jaa rhe hai)
        TreeNode* root = new TreeNode(element);
        int index = valMap[element];

        //right call pehle postorder mey
    ...
    TreeNode* buildTree(vector<int>& inorder, vector<int>& postorder) {
        ...
        TreeNode* root = constructTree(valMap, postorder, inorder, postIndex, inorderStart, inorderEnd, size);//size paas karne ki need nhi hai
        return root;
    }
};